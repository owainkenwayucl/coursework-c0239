 ...
 *   mpic++ -o rung rung.cpp

 * You can then run the program with
 *   mpirun -np [procs] rung
 *
 * Dr Owain Kenway, UCL
 */
#include <iostream>
...
int main(int argc, char **argv) {
	...
	MPI_Irecv(&rmessage, 1, MPI_INTEGER, last_proc, 0, MPI_COMM_WORLD, &request);

	MPI_Ssend(&message, 1, MPI_INTEGER, next_proc, 0, MPI_COMM_WORLD);
